...
func ParseArgs(config *configuration.GatewayConfig) error {
	...
	flag.StringVar(&config.TargetLoaders.JSONFile, "SimpleFile", "", "Simple YAML file containing the target configurations")
	flag.DurationVar(&config.TargetLoaders.JSONFileReloadInterval, "SimpleFileReloadInterval", 30*time.Second, "Interval to reload the simple YAML file containing the target configurations")
