...
func ParseArgs(config *configuration.GatewayConfig) error {
	...
	flag.StringVar(&config.Exporters.FluentHost, "ExportersFluentHost", "", "Fluentd exporter host address")
	flag.IntVar(&config.Exporters.FluentPort, "ExportersFluentPort", 8130, "Fluentd exporter host port")
	flag.BoolVar(&config.Exporters.FluentMarshalJSON, "ExportersFluentMarshalJSON", true, "Tells fluentd exporter if output should be marshaled as JSON (default: true)")
